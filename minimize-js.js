...
function NullActuator() {
  //魔改一波变成不移动格子的
  HTMLActuator.call(this)
}
NullActuator.prototype=Object.create(HTMLActuator.prototype);
NullActuator.prototype.actuate = function (grid, metadata) {
  return null;
};
...
KeyboardInputManager.prototype.listen = function () {
  ...
  this.bindButtonPress(".AI-button",startAgent)//增加AI代理按钮
...
function NullInputManager(){
  // 继承父类的属性
  KeyboardInputManager.call(this);
}

NullInputManager.prototype = Object.create(KeyboardInputManager.prototype);
NullInputManager.prototype.listen = function () {
  return null;
}
...
//创造一个假的存储管理器
function NullStorageManager(){
  LocalStorageManager.call(this);
}

NullStorageManager.prototype=Object.create(LocalStorageManager.prototype);
NullStorageManager.storage=window.fakeStorage;
...
//开始爆改
function newEmulator(GM){//创建一个模拟对象，为了节约开销，用完记得delete对象销毁
  var Emulator = new GameManager(GM.size,NullInputManager,NullActuator,NullStorageManager);
  Emulator.grid=new Grid(GM.size,GM.grid.serialize().cells)
  Emulator.score=GM.score
  return Emulator;
}

function sleep(ms) {//休眠函数，记得加await sleep()调用
...
function calculateBestMove(gameManager,depth=0){
  ...
  return [bestMove,bestGrade]
}

function countEmptyTiles(array){
  let count = 0;
...
function evaluateMove(Emulator, move,depth){
  var prev_score=Emulator.score
  if(Emulator.move(move)){//走位是否合法
